...
parser.add_argument('--probability_map_dir', type=str, default='', help='')
...
def test(trace_id, user_sample_id, probability_map_dir, seeds):
    ...
    while True:
        ...
        estimate_bw = solution.estimate_bw / 1000.
        target_bitrate = solution.target_bitrate / 1000.
        time_in_s = real_time / 1000.
        if sleep_time == 0:
            print(f'{time_in_s} {estimate_bw} {target_bitrate}', file=log_file_prediction_bw)
...
def test_all_traces(trace, user_sample_id, probability_map_dir):

    LOG_DIR = 'logs/log_dashlet/' + str(args.chunklength) + '/' + args.sub_dataset + '/user_' + str(user_sample_id) + '/'
    ...
    global log_file, log_file_prediction_bw
    for i in range(len(all_cooked_time)):
        LOG_FILE = LOG_DIR + 'trace_' + str(i)
        log_file = open(LOG_FILE, 'w')

        LOG_FILE_prediction_bw = LOG_DIR + 'prediction_bw_trace_' + str(i)
        log_file_prediction_bw = open(LOG_FILE_prediction_bw, 'w')

        print('------------trace ', i, '--------------')
        avg += test(i, user_sample_id, probability_map_dir, seeds)
        print('---------------------------------------\n\n')
...
def test_user_samples(trace, sample_cnt, probability_map_dir):  # test 50 user sample
    '''
    ...
    for j in range(sample_cnt):
        ...
        avgs += test_all_traces(trace, j, probability_map_dir)
    avgs /= sample_cnt
    print("\nScore: ", avgs[0])
    ...
    print("Rebuffer_all: ", avgs[6])

    performance_under_sub_dataset = './dashlet_' + str(args.chunklength/ 1000) + trace
...
if __name__ == '__main__':
    ...
    avgs = test_user_samples(args.trace, 5, args.probability_map_dir)