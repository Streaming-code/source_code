...
def test(trace_id, user_sample_id, BM_model_path, BA_model_path, seeds):

    # start the test
    from solution_Incendio import Algorithm
    solution = Algorithm()
    solution.Initialize(BM_model_path, BA_model_path)
...
def test_all_traces(trace, user_sample_id, BM_model_path, BA_model_path):
    avg = np.zeros(7)
    ...
    for i in range(len(all_cooked_time)):
        print('------------trace ', i, '--------------')
        avg += test(i, user_sample_id, BM_model_path, BA_model_path, seeds)
        print('---------------------------------------\n\n')
...
def test_user_samples(trace, sample_cnt, BM_model_path, BA_model_path):  # test 50 user sample
    '''
    ...
    for j in range(sample_cnt):
        ...
        avgs += test_all_traces(trace, j, BM_model_path, BA_model_path)
    avgs /= sample_cnt
    print("\nScore: ", avgs[0])
    ...
    performance_under_sub_dataset = './incendio_' + str(args.chunklength/ 1000) + trace
    with open(performance_under_sub_dataset, 'a') as f:
        # f.write(args.sub_dataset + '\n')
        f.write(f'{avgs[0]},{avgs[1]},{avgs[2]},{avgs[3]},{avgs[4]},{avgs[5]},{avgs[6]}\n')
...
if __name__ == '__main__':
    ...
    epoch = args.epoch
    BM_MODEL_SAVE_PATH = nn_model_save_path + 'bm_agent/bm_actor_epoch_' + epoch + '.pth'

    BA_MODEL_SAVE_PATH = nn_model_save_path + 'ba_agent/ba_actor_epoch_' + epoch + '.pth'

    avgs = test_user_samples(args.trace, 5, BM_MODEL_SAVE_PATH, BA_MODEL_SAVE_PATH)