...
VIDEO_CHUNCK_LEN = None

class Environment:
    def __init__(self, user_sample_id, all_cooked_time, all_cooked_bw, video_num, seeds, args):
        global VIDEO_CHUNCK_LEN
        VIDEO_CHUNCK_LEN = args.chunklength
        ...
        # USER_FILE = args.sample_user_dir + '/user_' + str(user_sample_id) + '.txt'
        # self.user_file = open(USER_FILE, 'wb')
        # USER_FILE_PROP = 'logs/sample_user_prop/user_'+str(user_sample_id)+'.txt'
        # USER_FILE_PROP = args.sample_user_prop_dir + '/user_' + str(user_sample_id) + '.txt'
        # self.user_file_prop = open(USER_FILE_PROP, 'wb')
        ...
        for p in range(PLAYER_NUM):
            ...
            self.players.append(Player(p, self.args.video_size_dir, self.args.user_ret_dir, self.args.chunklength))
            user_time, user_retent_rate = self.players[-1].get_user_model()
            self.user_models.append(Retention(user_time, user_retent_rate, seeds[self.video_cnt], VIDEO_CHUNCK_LEN))
            ...
            # self.user_file_prop.write((str(self.user_models[-1].get_ret_duration() / self.args.chunklength / (len(user_time) - 2)) + '\n').encode())
            # self.user_file_prop.flush()
    ...
    def player_op(self, operation):
        if operation == NEW:
            ...
            if self.video_cnt >= self.video_num:  # If exceed video cnt, no add
                return
            self.players.append(Player(self.video_cnt, self.args.video_size_dir, self.args.user_ret_dir, self.args.chunklength))
            self.end_video_id += 1
            # print("adding: ", self.video_num)
            user_time, user_retent_rate = self.players[-1].get_user_model()
            self.user_models.append(Retention(user_time, user_retent_rate, self.seeds[self.video_cnt], VIDEO_CHUNCK_LEN))
            self.video_cnt += 1
            self.total_watched_len += self.user_models[-1].get_ret_duration()  # sum the total watch duration
