...
class Player:
    ...
    def __init__(self, video_num, video_size_file, user_ret, chunklength):
        ...
        idx = 0
        with open(USER_RET + '../video_names.csv') as f:
            for line in f:
                if idx == video_num:
        ...
        self.chunk_num = int((self.video_len + VIDEO_CHUNCK_LEN - 1) / VIDEO_CHUNCK_LEN)
        # print(VIDEO_CHUNCK_LEN)

        for bitrate in range(len(VIDEO_BIT_RATE)):
            self.video_size[bitrate] = []
            # file_name = VIDEO_SIZE_FILE + self.video_name + '/video_size_' + str(bitrate)
            # with open(file_name) as f:
            #     for line in f:
            #         self.video_size[bitrate].append(int(line.split()[0])/VIDEO_SIZE_SCALE)
            for i in range(self.chunk_num):
                self.video_size[bitrate].append(VIDEO_BIT_RATE[bitrate] * VIDEO_CHUNCK_LEN / 1000. * 1000. / 8) # B
        ...
        with open(USER_RET + self.video_name) as file:
            for line in file:
